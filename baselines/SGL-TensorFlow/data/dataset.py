...
#             train_data, test_data = self._split_data(ori_prefix, saved_prefix, columns, by_time, config)
        ...
        self.test_matrix = csr_matrix((test_ratings, (test_data["user"], test_data["item"])),
                                      shape=(self.num_users, self.num_items))
        # ipdb.set_trace()
        self.cache_test_data = test_data
    ...
    def _split_data(self, ori_prefix, saved_prefix, columns, by_time, config):
        ...
        # ipdb.set_trace()
        rating_file = ori_prefix + "/data.csv"
        all_data = pd.read_csv(rating_file, sep=",",)
        all_data.columns = ["user", "item", "rating", "is_test", "is_tune", "is_train"]
        train_data = all_data.query("is_train==True")
        test_data = all_data.query("is_test==True")
        ...
        self.idmap_user2id = user2id
        self.idmap_item2id = item2id
        
        self.test_dataset_bc = pd.read_csv(ori_prefix + "/test_dataset_bc.csv", index_col=0)
        # ipdb.set_trace()
        # user_id  item_id  rating  is_test is_tune is_train  is_positive
        self.test_dataset_bc['user_id'] = self.test_dataset_bc['user_id'].map(self.userids)
        self.test_dataset_bc['item_id'] = self.test_dataset_bc['item_id'].map(self.itemids)
